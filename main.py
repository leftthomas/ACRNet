...
def test_loop(net, data_loader, step):
    net.eval()
    metric_info = {}
    with torch.no_grad():
        ...
        for feat, label, video_name, num_seg in tqdm(data_loader, initial=1, dynamic_ncols=True):
            ...
            act_score, _, seg_score = net(feat)
            # [C],  [T, C]
            ...
            for i, status in enumerate(pred):
                if status:
                    ...
                    for threshold in args.score_th:
                    ...
                    # temporal nms
                    proposal_dict[i] = temporal_nms(np.array(proposal_dict[i]), args.iou_th).tolist()
            results['results'][video_name] = result2json(proposal_dict, data_loader.dataset.idx_to_class)

        test_acc = num_correct / num_total

        gt_path = os.path.join(args.save_path, '{}_gt.json'.format(args.data_name))
        with open(gt_path, 'w') as json_file:
            json.dump(data_loader.dataset.annotations, json_file, indent=4)
        pred_path = os.path.join(args.save_path, '{}_pred.json'.format(args.data_name))
        ...
        desc = 'Test Step: [{}/{}] Test ACC: {:.1f} mAP@AVG: {:.1f}'.format(step, args.num_iter, test_acc * 100,
                                                                            m_ap_avg * 100)
        ...
        for i in range(args.map_th.shape[0]):
            ...
            metric_info['mAP@{:.2f}'.format(args.map_th[i])] = round(m_ap[i] * 100, 1)
        print(desc)
        return metric_info
...
if __name__ == '__main__':
    ...
    if args.model_file:
        ...
        for step in train_bar:
            model.train()
            feat, label, video_key, num_seg = next(train_loader)
