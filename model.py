...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # [N, T, C]
        seg_score = F.conv1d(F.normalize(out, dim=1), F.normalize(self.proxy, dim=1)).permute(0, 2, 1)
        ...
        # [N, C]
        act_score = torch.mean(torch.softmax(top_score / self.temperature, dim=-1), dim=1)
        bkg_score = torch.mean(torch.softmax(bottom_score / self.temperature, dim=-1), dim=1)
        # [N, K*C, D]
